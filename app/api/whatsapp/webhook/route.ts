...
import { claimWebhook, completeWebhook, recentChatHistory, releaseWebhook, saveBotMessage, saveConfirmedOrder } from '@/lib/botOrdersDb';
import { getServerWhatsAppConfigAsync } from '../config/route';

export async function GET() {
  return NextResponse.json({ status: 'online', service: 'WhatsApp booking webhook' });
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const history = await recentChatHistory(customerPhone, config.historyMessages ?? 16);
    ...
    if (!reply) { await releaseWebhook(messageId); return NextResponse.json({ success: true, ignored: true }); }
    ...
    if (!response.ok || !result.idMessage) {
      await releaseWebhook(messageId);
      return NextResponse.json({ success: false, error: 'WhatsApp send failed' }, { status: 502 });
    }
    await completeWebhook(messageId);
    await saveBotMessage(customerPhone, text, 'inbound', customerName);
    await saveBotMessage(customerPhone, reply, 'outbound', 'بوت الرد الآلي');
